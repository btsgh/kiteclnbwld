...
export function Navbar() {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-[60] transition-colors duration-300 ${isScrolled || mobileMenuOpen ? 'bg-[#FDF0EB] shadow-sm' : 'bg-[#FDF0EB]/90 backdrop-blur-sm'}`}>
      ...
      <div className={`fixed inset-0 z-50 transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`absolute top-0 right-0 w-4/5 max-w-sm bg-[#FDF0EB] h-full shadow-xl transition-transform duration-300 flex flex-col pt-24 px-6 ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
